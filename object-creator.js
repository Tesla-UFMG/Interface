...
class ObjectCreator {
  constructor(databaseName) {
    ...
    this.idsMap = fields.reduce(
      (obj, item) => ((obj[item.id + "-" + item.pos] = item.index), obj),
      {}
    );
  ...
  async treatInfo(id, data) {
    if (this.isPackRelated(id)) {
      let packIndex = parseInt((id - 300) / 5);
      let packPart = (id - 300) % 5;
      ...
      if (id == cFields.current0.id) {
        ...
        this.insertData(
          this.mapId(cFields.simCharge.id, cFields.simCharge.pos),
          this.cargaHandler.getChargeMean()
        );
      }
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        // console.log("Informação inválida! "+id.toString(16)+"-"+i);
          ...
          this.insertData(fieldId, data[i]);
          const t =
            constants.fieldsLimit.fieldsLimit[
              constants.mappings.fieldIdNameMap[fieldId]
            ];
          if (typeof t !== "undefined") {
            let indexF = this.offLimitFields.findIndex(
            ...
            if (
              (typeof t.max !== "undefined" &&
                parseInt(data[i]) >=
                  t.max *
                    (1 -
                      constants.fieldsLimit.percentageLimitTolerance / 100)) ||
              (typeof t.min !== "undefined" &&
                parseInt(data[i]) <=
                  t.min *
                    (1 + constants.fieldsLimit.percentageLimitTolerance / 100))
            ) {
  ...
  buildDatalog() {
    ...
    constants.datalog.datalogOrder.forEach((value, index) => {
      ...
      datalogStr +=
        index === constants.datalog.datalogOrder.length - 1 ? "\r\n" : "\t";
    }, this);
